...
def pobeda():
    try:
        ...
        url = f"https://flysmartavia.com/search/LED-2503-SVX-1"
        driver.get(url)
        ...
        time.sleep(10)
        table = driver.find_elements(By.CLASS_NAME, "js-variant-card-wrapper")
        for el in table:
            fin = el.find_elements(By.CLASS_NAME, "inner")
            tin = el.find_elements(By.CLASS_NAME, "name")
            ttime = fin[0].find_element(By.CLASS_NAME, 'time')
            print(ttime.text)
            #print(time_from + " - " + time_to + plusday)
            print(tin[0].text)
            ftime = fin[1].find_element(By.CLASS_NAME, 'time')
            print(ftime.text)
            # print(time_from + " - " + time_to + plusday)
            print(tin[1].text)
            price = el.find_element(By.CLASS_NAME, 'button-wrapper')
            print(price.text)
            terminal = ""
            compname = "Победа"
            leftsit = ""
