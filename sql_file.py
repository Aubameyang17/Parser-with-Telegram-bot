def info_to_table(name, time_from, airfrom, terminal, timeto, plusday, airto, toterminal, compname, price, leftsit, cursor, conn):
    parametrs = (time_from, airfrom, terminal, timeto, plusday, airto, toterminal, compname, price, leftsit)
    cursor.execute('INSERT INTO ' + name + ' (time_from, airfrom, terminal_from, time_to, plusday, airto, terminal_to, '
                   'name, price, left_sit) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', parametrs)
    conn.commit()
...
def take_orders(name, userid, airfrom, airto, usermonth, userdate, cursor, conn):
    ...
    cursor.execute('INSERT INTO orders (id_user, airfrom, airto, month, lowprice, userdate) VALUES (%s, %s, %s, %s, %s, %s)', parametrs)
    conn.commit()
...
def create_table(userid, cursor, conn):
    ...
    if res:
        ...
        cursor.execute("CREATE TABLE if not exists " + name + " (id SERIAL PRIMARY KEY, time_from VARCHAR(50),  "
                   "airfrom VARCHAR(50), terminal_from VARCHAR(50), time_to VARCHAR(50), plusday VARCHAR(50), "
                   "airto VARCHAR(50), terminal_to VARCHAR(50), name VARCHAR(50), price integer, "
